...
var showTopAnswerer = function (answerer) {
    ...
    // clone our result template code
    var result = $('.chickenfingers .topanswerers').clone();
    ...
    //reputation score
    var repScore = result.find('.reputation');
    repScore.text(answerer.user.reputation);
...
// takes a string of semi-colon separated tags to be searched
// for on StackOverflow
var getTopAnswerers = function (top) {

    // the parameters we need to pass in our request to StackOverflow's API
    var request = {
        //tags: top, // not used, 'tags' is actually in the URL, not as a search term appended to URL
        site: 'stackoverflow',
    };
    ...
    $.ajax({
        ...
        .done(function (result) { //this waits for the ajax to return with a succesful promise object
            var searchResults = showSearchResults(top, result.items.length);
            $('.search-results').html(searchResults);
            //$.each is a higher order function. It takes an array and a function as an argument.
            //The function is executed once for each item in the array.
            $.each(result.items, function (i, item) {
...
$(document).ready(function () {
    ...
    $('.inspiration-getter').submit(function (event) {
        ...
        $('.results').html('');
        // get the value of the tags the user submitted
        var top = $(this).find("input[name='top']").val();
        getTopAnswerers(top);
